...
const Header: FunctionComponent = observer(() => {
  ...
  return (
    <header>
      <nav>
        <ul>
          {Object.entries(store.bookings).map((b) => (
            <li key={b[0]}>{JSON.stringify(b[1])}</li>
          ))}
        </ul>
      </nav>
    </header>
...
const Comps: FunctionComponent = observer(() => {
  ...
  return (
    <>
      {store!.comps.map((c, idx) => (
        <Competition idx={idx} key={c.id} comp={c} players={store!.players} />
      ))}
    </>
  );
});

const Competition: FunctionComponent<{
  idx: number;
  comp: Comp;
  players: { [id: string]: string };
}> = observer(({ idx, comp, players }) => {
  const store = useContext(StoreContext);
  ...
  return (
    <details>
      <summary>
        {formatDateTime(comp.comp_date)}
        {bookings.length &&
          `, ${bookings[0].booking_times}-${bookings[0].player_ids.map(
            (p_id) => players[p_id]
          )}`}
        <p>
          <sub>{comp.html_description}</sub>
        </p>
      </summary>
      <small>Notes: {comp.notes}</small>
      <br />
      ...
      <form id={`comp-${idx}`}>
        ...
        {tee_times.map((t) => {
          return (
            <select className="tee-time" key={t} onChange={handleSetTeeTime}>
          ...
          <>
            ...
            <label>
              ...
              <button onClick={testPassword}>
                Test password (Must do before booking)
              </button>
            </label>
            <label>
              Partners:
              <small>
                <br />
                Order matters, for example, with a 2 ball, Rhys is the default,
                swap him for someone else if necessary
              </small>
              <Partner
                partner={partner1}
            ...
            {store.password_is_legit && (
              <button onClick={handleSubmitJob}>Book</button>
            )}
          </>
        )}
      </form>
...
const Partner = ({ current_players, partner, setPartner }) => {
  ...
  return (
    <select className="partners" onChange={(e) => setPartner(e.target.value)}>
      <option key={"selected"} value={partner}>
        {players[partner]}
      </option>
    ...
    <>
      {Object.entries(players)
        ...
        .sort((a, b) => {
          const [keya, vala] = a;
          const [keyb, valb] = b;
          if (preferred_players.includes(keya)) {
            return -1;
          }
          if (preferred_players.includes(keyb)) {
            return 1;
          }
          if (vala < valb) {
            return -1;
...
const formatDateTime = (time: number | null, include_time = false): string => {
  ...
  return include_time
    ? dt.toDateString() +
        " - " +
        dt.toLocaleTimeString("en-GB", { timeZone: "Europe/London" })
    : dt.toDateString();
};
    ...
    return fetch(`${this.url}/scheduler/booking/`, {
      ...
      .then((response) => {
        if (!response.ok) {
          ...
          alert(
            "Error booking comp, get in touch with joe, take a screenshot if you can"
          );
        }
        return response;
      })
...
class Store {
  ...
  async book_comp(
    ...
    const data = await this.api.book_comp(
      comp_id,
      booking_times,
      player_ids,
      username,
