...
class MasterScoreboard:
    ...
    def choose_slot(self, block_id_pair, form_data):
        next_page = 'https://www.masterscoreboard.co.uk/bookings1/BookSelectPartners.php?CWID=5070'

        payload = {
            k: v for k, v in form_data.values() if k.startswith('BlockNumAvailable')
        }
    ...
    def choose_partner(self, partner_ids, form_data):
        next_page = 'https://www.masterscoreboard.co.uk/bookings1/BookTime.php'

        payload = form_data
        payload.update({
            f"Partner_{idx}": str(p_id) for idx, p_id in enumerate(partner_ids)
        })
        payload['Book'] = 'Book'
